...
class YunafiedApiClient {
  ...
  async getCyclingQuestStats(): Promise<{ bestScore: number; coinBalance: number }> {
    return this.request('/api/cycling-quest/stats');
  }
  ...
  async getAdminAnalytics(filters?: { dateFrom?: string; dateTo?: string; status?: string }, refresh = false): Promise<AdminAnalyticsItem> {
  ...
  async submitAssessment(id: string, answers: Array<{ questionId: string; selectedChoiceId?: string|null; answerText?: string|null }>): Promise<{ score: number; totalPoints: number; submittedAt: string; review?: Array<{ questionId: string; question: string; answer: string; isCorrect: boolean; correctAnswer: string }> }> { return this.request(`/api/assessments/assignments/${id}/submit`, { method: 'POST', body: JSON.stringify({ answers }) }); }
