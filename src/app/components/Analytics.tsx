...
export function Analytics({ onNavigateView, submissions = [], assignments = [], users = [], schedules = [], generatedBy }: { onNavigateView?: (view: string) => void; submissions?: SubmissionItem[]; assignments?: AssignmentItem[]; users?: AuthUser[]; schedules?: ScheduleItem[]; generatedBy: AuthUser }) {
  ...
  const overallSummary = data ? `The system currently covers ${data.totalStudents} student${data.totalStudents === 1 ? '' : 's'} and ${data.totalTeachers} teacher${data.totalTeachers === 1 ? '' : 's'}, with ${data.totalSessions} session${data.totalSessions === 1 ? '' : 's'} and ${data.totalSubmissions} submission${data.totalSubmissions === 1 ? '' : 's'} recorded. ${data.gradeDistribution.length ? 'The grade distribution and student-performance charts show where learning support is most needed.' : 'More graded work is needed before performance patterns can be interpreted confidently.'}` : '';
  return <div className="p-6 max-w-7xl mx-auto print:p-0">
    ...
    {loading && !data ? <div className="py-24 text-center text-gray-400">Loading dashboard…</div> : data && <>
      ...
      <section className="mb-6 grid grid-cols-1 lg:grid-cols-2 gap-4 print:hidden">
        ...
        <div className="rounded-xl border border-emerald-100 bg-emerald-50 p-5"><h3 className="font-semibold text-emerald-900">How the admin can use this</h3><p className="mt-2 text-sm leading-6 text-emerald-900/80">Use enrollment and teacher-activity charts to balance workloads, use session trends to plan capacity, and use grade and submission charts to prioritize teacher support or student interventions. Open any metric to review the underlying records before making changes.</p></div>
      </section>
    ...
    <div className="admin-print-report" aria-hidden="true">
      <div className="border-b-2 border-indigo-600 pb-4 mb-6"><h1 className="text-2xl font-bold text-gray-900">YUNAfied Admin Dashboard Report</h1><p className="text-sm text-gray-500 mt-1">Overview and Performance Analytics · Generated {new Date().toLocaleString()} · Generated by {generatedBy.role} {generatedBy.fullName}</p></div>
