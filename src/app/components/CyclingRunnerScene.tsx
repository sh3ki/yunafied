...
export function CyclingRunnerScene({ lane, paused, powerUp, treasureVisible, finishLine, onCoin, onPower, onPowerMiss }: Props) {
  ...
  useEffect(() => {
    ...
    scene.background = new THREE.Color('#8bc6de');
    scene.fog = new THREE.Fog('#9ecbd7', 18, 115);
    ...
    scene.add(new THREE.HemisphereLight('#eaf8ff', '#254c30', 2.25));
    const sun = new THREE.DirectionalLight('#ffe2a1', 2.8); sun.position.set(-9, 14, 4); scene.add(sun);
    const world = new THREE.Group(); scene.add(world);
    const ground = new THREE.Mesh(new THREE.PlaneGeometry(70, 180), new THREE.MeshLambertMaterial({ color: '#476f46' })); ground.rotation.x = -Math.PI / 2; ground.position.z = -70; world.add(ground);
    const road = new THREE.Mesh(new THREE.PlaneGeometry(6.5, 180), new THREE.MeshLambertMaterial({ color: '#b77b42' })); road.rotation.x = -Math.PI / 2; road.position.set(0, .012, -70); world.add(road);
    ...
    const animate = (now: number) => { frame = requestAnimationFrame(animate); const delta = Math.min(.05, (now - last) / 1000); last = now; elapsed += delta;
      if (!pausedRef.current) {
        ...
        if (powerGroup.visible) { powerGroup.position.z += speed; powerGroup.rotation.y += delta * 3; powerGroup.position.y = .62 + Math.sin(elapsed * 5) * .12; if (powerGroup.position.z > 6) { powerGroup.visible = false; if (powerGroup.userData.lane === laneRef.current && activePower) callbacks.current.onPower(); else callbacks.current.onPowerMiss(); activePower = null; } }
        if (treasureRef.current && !treasureActive) { treasureGroup.visible = true; treasureGroup.position.set(0, 0, -48); treasureActive = true; }
