...
type AvailabilityDraft = { dayOfWeek: number; date?: string | null; startTime: string; endTime: string };
const todayIso = () => new Date().toISOString().slice(0, 10);
function AvailabilityEditor({ blocks, setBlocks }: { blocks: AvailabilityDraft[]; setBlocks: React.Dispatch<React.SetStateAction<AvailabilityDraft[]>> }) {
  const add = () => setBlocks((items) => [...items, { dayOfWeek: new Date(`${todayIso()}T00:00:00`).getDay(), date: todayIso(), startTime: '09:00', endTime: '10:00' }]);
  return <div className="rounded-xl border border-violet-100 bg-violet-50/40 p-3 md:col-span-2 lg:col-span-3"><div className="flex items-center justify-between"><div><p className="font-semibold text-gray-700">Teacher Availability</p><p className="text-xs text-gray-500">Choose a specific date or a recurring day of the week.</p></div><button type="button" onClick={add} className="rounded-lg bg-violet-600 px-3 py-1.5 text-xs font-semibold text-white">Add availability</button></div><div className="mt-2 space-y-2">{blocks.map((block, index) => <div key={`${block.date || 'weekly'}-${block.dayOfWeek}-${index}`} className="flex flex-wrap items-center gap-2"><select value={block.date ? 'date' : 'day'} onChange={(e) => setBlocks((items) => items.map((item, i) => { if (i !== index) return item; const date = e.target.value === 'date' ? (item.date || todayIso()) : null; return { ...item, date, dayOfWeek: date ? new Date(`${date}T00:00:00`).getDay() : item.dayOfWeek }; }))} className="rounded-lg border px-2 py-2 text-sm"><option value="date">Specific date</option><option value="day">Every week</option></select>{block.date ? <input type="date" value={block.date} onChange={(e) => setBlocks((items) => items.map((item, i) => i === index ? { ...item, date: e.target.value || null, dayOfWeek: e.target.value ? new Date(`${e.target.value}T00:00:00`).getDay() : item.dayOfWeek } : item))} className="rounded-lg border px-2 py-2 text-sm" /> : <select value={block.dayOfWeek} onChange={(e) => setBlocks((items) => items.map((item, i) => i === index ? { ...item, dayOfWeek: Number(e.target.value) } : item))} className="rounded-lg border px-2 py-2 text-sm">{['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map((day, value) => <option key={day} value={value}>{day}</option>)}</select>}<input type="time" value={block.startTime} onChange={(e) => setBlocks((items) => items.map((item, i) => i === index ? { ...item, startTime: e.target.value } : item))} className="rounded-lg border px-2 py-2 text-sm" /><span className="text-gray-400">to</span><input type="time" value={block.endTime} onChange={(e) => setBlocks((items) => items.map((item, i) => i === index ? { ...item, endTime: e.target.value } : item))} className="rounded-lg border px-2 py-2 text-sm" /><button type="button" onClick={() => setBlocks((items) => items.filter((_, i) => i !== index))} className="px-2 text-sm text-red-500">Remove</button></div>)}</div></div>;
}
...
export function EnrollmentRecords({ role, generatedBy, onAddUser, onEditUser, onDeleteUser, onUploadProfileImage, onChangeUserStatus }: EnrollmentRecordsProps) {
