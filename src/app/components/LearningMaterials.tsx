...
export function LearningMaterials({ role, backendBaseUrl, subjectOptions = [] }: LearningMaterialsProps) {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      {previewItem && <div className="fixed inset-0 z-50 bg-slate-950/70 p-0 sm:p-6 flex items-center justify-center" role="dialog" aria-modal="true" aria-labelledby="material-preview-title" onMouseDown={(event) => { if (event.target === event.currentTarget) setPreviewItem(null); }}><div className="bg-white w-full h-full max-w-6xl rounded-none sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col"><div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 px-3 sm:px-4 py-3 border-b bg-white"><div className="min-w-0 max-w-full sm:max-w-[45%]"><h2 id="material-preview-title" className="font-semibold text-gray-900 truncate">{previewItem.title}</h2><p className="text-xs text-gray-500 truncate">{previewItem.fileName || previewItem.resourceUrl}</p></div><div className="flex items-center justify-end gap-1.5 sm:gap-2 shrink-0 flex-wrap w-full sm:w-auto">{previewKind === 'pdf' && <><button aria-label="Previous page" disabled={previewPage <= 1} onClick={() => setPreviewPage((page) => Math.max(1, page - 1))} className="p-1.5 sm:p-2 rounded-lg border disabled:opacity-40 hover:bg-gray-50"><ChevronLeft className="h-4 w-4" /></button><span className="text-xs text-gray-600 min-w-14 text-center">Page {previewPage}</span><button aria-label="Next page" onClick={() => setPreviewPage((page) => page + 1)} className="p-1.5 sm:p-2 rounded-lg border hover:bg-gray-50"><ChevronRight className="h-4 w-4" /></button><button aria-label="Zoom out" disabled={previewZoom <= 50} onClick={() => setPreviewZoom((zoom) => Math.max(50, zoom - 10))} className="p-1.5 sm:p-2 rounded-lg border disabled:opacity-40 hover:bg-gray-50"><ZoomOut className="h-4 w-4" /></button><span className="text-xs text-gray-600">{previewZoom}%</span><button aria-label="Zoom in" disabled={previewZoom >= 200} onClick={() => setPreviewZoom((zoom) => Math.min(200, zoom + 10))} className="p-1.5 sm:p-2 rounded-lg border disabled:opacity-40 hover:bg-gray-50"><ZoomIn className="h-4 w-4" /></button></>}{previewItem.materialType === 'file' && <button onClick={() => download(previewItem)} className="inline-flex items-center gap-1 rounded-lg bg-violet-600 text-white px-2.5 sm:px-3 py-2 text-sm hover:bg-violet-700"><Download className="h-4 w-4" /><span className="hidden xs:inline sm:inline">Download</span></button>}<button aria-label="Close preview" onClick={() => setPreviewItem(null)} className="p-1.5 sm:p-2 rounded-lg text-gray-600 hover:bg-gray-100"><X className="h-5 w-5" /></button></div></div><div className="relative flex-1 min-h-0 bg-slate-100">{previewLoading && <div className="absolute inset-0 z-10 flex items-center justify-center bg-slate-100/90 text-gray-600"><Loader2 className="h-5 w-5 animate-spin mr-2" />Loading preview…</div>}{previewError && <div className="h-full flex items-center justify-center p-6 text-center"><div><AlertCircle className="h-8 w-8 text-rose-500 mx-auto mb-2" /><p className="font-medium text-gray-800">We couldn’t load this preview.</p><p className="text-sm text-gray-600 mt-1">You can download the original file to view it.</p></div></div>}{!previewError && previewKind === 'unsupported' && <div className="h-full flex items-center justify-center p-6 text-center"><div><FileText className="h-10 w-10 text-violet-500 mx-auto mb-3" /><p className="font-medium text-gray-800">Preview isn’t available for this file type.</p><p className="text-sm text-gray-600 mt-1">You can download the file to view it.</p></div></div>}{!previewError && previewKind === 'link' && <div className="h-full flex items-center justify-center p-6 text-center"><div><LinkIcon className="h-10 w-10 text-violet-500 mx-auto mb-3" /><p className="font-medium text-gray-800">This material is an external link.</p><a href={previewUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 mt-4 rounded-lg bg-violet-600 text-white px-4 py-2 text-sm hover:bg-violet-700">Open link <ExternalLink className="h-4 w-4" /></a></div></div>}{!previewError && previewKind === 'image' && <div className="h-full overflow-auto flex items-center justify-center p-4"><img src={previewUrl} alt={previewItem.title} onLoad={() => setPreviewLoading(false)} onError={() => { setPreviewLoading(false); setPreviewError(true); }} className="max-w-full max-h-full object-contain shadow-sm" /></div>}{!previewError && previewKind === 'pdf' && <div className="h-full overflow-auto flex items-center justify-center p-4"><img src={pdfViewerUrl} alt={`Page ${previewPage} of ${previewItem.title}`} onLoad={() => setPreviewLoading(false)} onError={() => { setPreviewLoading(false); setPreviewError(true); }} className="max-w-none max-h-full object-contain shadow-sm" /></div>}{!previewError && previewKind === 'office' && <iframe title={`Preview of ${previewItem.title}`} src={officeViewerUrl} onLoad={() => setPreviewLoading(false)} onError={() => { setPreviewLoading(false); setPreviewError(true); }} className="w-full h-full border-0 bg-white" />}{!previewError && previewKind === 'text' && <iframe title={`Preview of ${previewItem.title}`} src={previewUrl} onLoad={() => setPreviewLoading(false)} onError={() => { setPreviewLoading(false); setPreviewError(true); }} className="w-full h-full border-0 bg-white" />}</div></div></div>}
    </div>
  );
}
