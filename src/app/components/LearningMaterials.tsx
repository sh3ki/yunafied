...
function getPreviewKind(item: LearningMaterialItem): 'pdf' | 'text' | 'office' | 'unsupported' | 'link' {
...
interface LearningMaterialsProps { role: UserRole; backendBaseUrl: string; }
...
export function LearningMaterials({ role, backendBaseUrl }: LearningMaterialsProps) {
  ...
  const subjects = useMemo(() => Array.from(new Set(rows.map((item) => item.subject))).sort(), [rows]);
  ...
  const openPreview = (item: LearningMaterialItem) => {
    ...
    setPreviewLoading(kind === 'pdf' || kind === 'text' || kind === 'office');
  };
  ...
  const pdfViewerUrl = `${previewUrl}#page=${previewPage}&zoom=${previewZoom}`;
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      {canManage && <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-4 mb-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3"><input value={form.title} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))} placeholder="Title" className="border rounded-lg px-3 py-2" /><input value={form.subject} onChange={(e) => setForm((p) => ({ ...p, subject: e.target.value }))} placeholder="Subject" className="border rounded-lg px-3 py-2" /><input value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} placeholder="Description (optional)" className="border rounded-lg px-3 py-2" />{mode === 'link' ? <input value={form.url} onChange={(e) => setForm((p) => ({ ...p, url: e.target.value }))} placeholder="https://..." className="md:col-span-2 border rounded-lg px-3 py-2" /> : <input type="file" accept=".pdf,.doc,.docx,.txt,.ppt,.pptx,.xls,.xlsx" onChange={(e) => setForm((p) => ({ ...p, file: e.target.files?.[0] || null }))} className="md:col-span-2 border rounded-lg px-3 py-2" />}<button onClick={mode === 'link' ? createLink : createFile} disabled={saving} className="bg-violet-600 hover:bg-violet-700 text-white rounded-lg px-3 py-2 inline-flex items-center justify-center gap-2 disabled:opacity-60"><Plus className="h-4 w-4" />{saving ? 'Saving…' : 'Add Material'}</button></div>
      </div>}
