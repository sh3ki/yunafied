  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans text-gray-900 pb-16 md:pb-0">
      ...
      <main className="flex-1 md:ml-64 overflow-y-auto">
        {(userRole === 'admin' || userRole === 'teacher') && <div className="print-generated-by" aria-hidden="true">Generated by {userRole} {session.user.fullName}</div>}
            ...
            >
              {currentView === 'dashboard' && (
                <div className="p-4 md:p-8">
                  ...
                  {userRole === 'admin' && <Analytics onNavigateView={onNavigateView} submissions={data.submissions} assignments={data.assignments} users={data.users} schedules={data.schedules} generatedBy={session.user} />}
              ...
              {currentView === 'enrollments' && <EnrollmentRecords role={userRole} generatedBy={session.user} onAddUser={onAddUser} onEditUser={onEditUser} onDeleteUser={onDeleteUser} onUploadProfileImage={onUploadProfileImage} onChangeUserStatus={onChangeUserStatus} />}

              {currentView === 'teacher-records' && userRole === 'admin' && <TeacherRecords generatedBy={session.user} />}

              {currentView === 'student-records' && (userRole === 'admin' || userRole === 'teacher') && <StudentRecords role={userRole} generatedBy={session.user} />}
              ...
              {currentView === 'audit-logs' && userRole === 'admin' && <AuditLogs generatedBy={session.user} />}

              {currentView === 'meeting-history' && userRole === 'admin' && <MeetingHistory generatedBy={session.user} />}
            </motion.div>
          </AnimatePresence>
        </div>
      </main>
