...
export class YunafiedService {
  ...
  async createAssessment(input: { title: string; subject: string; gradeLevel: string; assessmentType: 'pre'|'post'; pairedAssessmentId?: string|null; instructions?: string; questions: Array<{ type: string; prompt: string; points: number; choices?: Array<{ text: string; isCorrect: boolean }>; acceptedAnswers?: string[] }> }, requester: { id: string; role: UserRole }): Promise<unknown> {
    ...
    if (input.assessmentType === 'post') {
      if (!input.pairedAssessmentId) throw new Error('A post-assessment must be paired with a pre-assessment.');
      const pair = await pool.query(`SELECT subject,grade_level,assessment_type FROM assessments WHERE id=$1`, [input.pairedAssessmentId]);
      if (!pair.rows[0] || pair.rows[0].assessment_type !== 'pre' || pair.rows[0].subject !== input.subject || pair.rows[0].grade_level !== input.gradeLevel) throw new Error('Post-assessments must pair with a pre-assessment for the same subject and level.');
    }
    const client = await pool.connect();
    try { await client.query('BEGIN'); const created = await client.query(`INSERT INTO assessments (title,subject,grade_level,assessment_type,paired_assessment_id,instructions,created_by) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id`, [input.title.trim(), input.subject, input.gradeLevel, input.assessmentType, input.pairedAssessmentId || null, input.instructions || '', requester.id]); const id = created.rows[0].id;
      for (let i=0;i<input.questions.length;i++) { const q=input.questions[i]; const qr=await client.query(`INSERT INTO assessment_questions (assessment_id,question_order,question_type,prompt,points,accepted_answers) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id`, [id,i+1,q.type,q.prompt.trim(),q.points,q.acceptedAnswers || []]); for (let j=0;j<(q.choices || []).length;j++) await client.query(`INSERT INTO assessment_choices (question_id,choice_order,choice_text,is_correct) VALUES ($1,$2,$3,$4)`, [qr.rows[0].id,j+1,q.choices![j].text.trim(),q.choices![j].isCorrect]); }
      await client.query('COMMIT'); return this.getAssessment(id, requester, true);
    } catch (error) { await client.query('ROLLBACK'); throw error; } finally { client.release(); }
  }

  async publishAssessment(id: string, requester: { id: string; role: UserRole }): Promise<unknown> { const ownership = requester.role === 'admin' ? '' : `AND created_by='${requester.id}'`; const result = await pool.query(`UPDATE assessments SET status='published',updated_at=NOW() WHERE id=$1 ${ownership} RETURNING id`, [id]); if (!result.rows[0]) throw new Error('Assessment not found or not owned by you.'); return this.getAssessment(id, requester, true); }

  async assignAssessment(id: string, studentIds: string[], dueAt: string|null, requester: { id: string; role: UserRole }): Promise<unknown[]> {
    const assessment = await pool.query<{ subject:string }>(`SELECT subject FROM assessments WHERE id=$1 AND status='published'`, [id]); if (!assessment.rows[0]) throw new Error('Only published assessments can be assigned.');
