...
const assessmentShuffle = <T,>(items: T[], seed: string): T[] => {
  let value = 2166136261;
  for (const character of seed) value = Math.imul(value ^ character.charCodeAt(0), 16777619);
  return items.map((item, index) => ({ item, index, sort: Math.imul(value ^ index, 2246822519) }))
...
export class YunafiedService {
  ...
  async listAssessmentSubjects(requester: { id: string; role: UserRole }): Promise<string[]> {
    const params: unknown[] = [];
    let where = "e.status='active'";
    if (requester.role === 'teacher') { params.push(requester.id); where += ` AND e.teacher_id=$${params.length}`; }
    const result = await pool.query<{ subject: string }>(`SELECT DISTINCT e.subject FROM enrollment_records e WHERE ${where} ORDER BY e.subject`, params);
    return result.rows.map((row) => row.subject);
  }

  async listAssessments(requester: { id: string; role: UserRole }): Promise<unknown[]> {
    const params: unknown[] = requester.role === 'admin' ? [] : [requester.id];
    const visibility = requester.role === 'admin' ? '' : requester.role === 'teacher'
      ? `WHERE a.created_by=$1 OR EXISTS (SELECT 1 FROM assessment_assignments ax WHERE ax.assessment_id=a.id AND ax.teacher_id=$1)`
      : `WHERE EXISTS (SELECT 1 FROM assessment_assignments ax WHERE ax.assessment_id=a.id AND ax.student_id=$1)`;
    const result = await pool.query(`SELECT a.id,a.title,a.subject,a.grade_level AS "gradeLevel",a.assessment_type AS "assessmentType",a.paired_assessment_id AS "pairedAssessmentId",a.instructions,a.status,a.version,a.created_by AS "createdById",u.full_name AS "createdByName",a.created_at AS "createdAt",a.updated_at AS "updatedAt",COUNT(DISTINCT q.id)::int AS "questionCount",COUNT(DISTINCT ax.id)::int AS "assignedCount",COUNT(DISTINCT at.id) FILTER (WHERE at.status='submitted')::int AS "completedCount" FROM assessments a JOIN users u ON u.id=a.created_by LEFT JOIN assessment_questions q ON q.assessment_id=a.id LEFT JOIN assessment_assignments ax ON ax.assessment_id=a.id LEFT JOIN assessment_attempts at ON at.assignment_id=ax.id ${visibility} GROUP BY a.id,u.full_name ORDER BY a.updated_at DESC`, params);
    return result.rows;
  }

  async getAssessment(id: string, requester: { id: string; role: UserRole }, includeAnswers = false): Promise<unknown | null> {
    const list = await this.listAssessments(requester);
    const item = (list as Array<Record<string, unknown>>).find((row) => row.id === id);
    if (!item) return null;
    const questions = await pool.query(`SELECT q.id,q.question_order AS "questionOrder",q.question_type AS type,q.prompt,q.points,q.accepted_answers AS "acceptedAnswers",COALESCE(json_agg(json_build_object('id',c.id,'text',c.choice_text${includeAnswers ? ",'isCorrect',c.is_correct" : ''}) ORDER BY c.choice_order) FILTER (WHERE c.id IS NOT NULL),'[]') AS choices FROM assessment_questions q LEFT JOIN assessment_choices c ON c.question_id=q.id WHERE q.assessment_id=$1 GROUP BY q.id ORDER BY q.question_order`, [id]);
    if (!includeAnswers) questions.rows.forEach((q) => delete q.acceptedAnswers);
    const isPostForStudent = requester.role === 'student' && item.assessmentType === 'post';
    const shuffledQuestions = isPostForStudent ? assessmentShuffle(questions.rows, `${id}:${requester.id}`) : questions.rows;
    if (isPostForStudent) shuffledQuestions.forEach((q: any, index: number) => { q.choices = assessmentShuffle(q.choices, `${id}:${requester.id}:${q.id}`); q.questionOrder = index + 1; });
    return { ...item, answerReviewEnabled: true, questions: shuffledQuestions };
  }

  async createAssessment(input: { title: string; subject: string; gradeLevel?: string; instructions?: string; questions: Array<{ type: string; prompt: string; points: number; choices?: Array<{ text: string; isCorrect: boolean }>; acceptedAnswers?: string[] }> }, requester: { id: string; role: UserRole }): Promise<unknown> {
    const subjects = await this.listAssessmentSubjects(requester);
    if (!subjects.includes(input.subject)) throw new Error('Select a subject from your active enrollment subjects.');
    if (requester.role === 'teacher') {
      const owned = await pool.query(`SELECT 1 FROM enrollment_records WHERE teacher_id=$1 AND subject=$2 AND status='active' LIMIT 1`, [requester.id, input.subject]);
      if (!owned.rows[0]) throw new Error('Teachers can only create assessments for their assigned subjects.');
    }
    for (const question of input.questions) {
      if (question.type === 'true-false') {
        if (question.choices?.length !== 2 || question.choices.some((choice) => !['True', 'False'].includes(choice.text))) throw new Error('True or false questions must use True and False options.');
      } else if (question.type === 'multiple-choice') {
        if (!question.choices || question.choices.length < 2 || question.choices.filter((choice) => choice.isCorrect).length !== 1) throw new Error('Multiple choice questions need at least two options and one correct answer.');
      } else if (!question.acceptedAnswers?.some((answer) => answer.trim())) throw new Error('Identification questions need an answer.');
    }
    const client = await pool.connect();
    try { await client.query('BEGIN');
      const pre = await client.query(`INSERT INTO assessments (title,subject,grade_level,assessment_type,instructions,created_by) VALUES ($1,$2,$3,'pre',$4,$5) RETURNING id`, [input.title.trim(), input.subject, input.gradeLevel || '', input.instructions || '', requester.id]);
      const post = await client.query(`INSERT INTO assessments (title,subject,grade_level,assessment_type,paired_assessment_id,instructions,created_by) VALUES ($1,$2,$3,'post',$4,$5,$6) RETURNING id`, [`${input.title.trim()} · Post-assessment`, input.subject, input.gradeLevel || '', pre.rows[0].id, input.instructions || '', requester.id]);
      for (const assessmentId of [pre.rows[0].id, post.rows[0].id]) {
        for (let i=0;i<input.questions.length;i++) { const q=input.questions[i]; const qr=await client.query(`INSERT INTO assessment_questions (assessment_id,question_order,question_type,prompt,points,accepted_answers) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id`, [assessmentId,i+1,q.type,q.prompt.trim(),q.points,q.acceptedAnswers || []]); for (let j=0;j<(q.choices || []).length;j++) await client.query(`INSERT INTO assessment_choices (question_id,choice_order,choice_text,is_correct) VALUES ($1,$2,$3,$4)`, [qr.rows[0].id,j+1,q.choices![j].text.trim(),q.choices![j].isCorrect]); }
      }
      await client.query('COMMIT'); return this.getAssessment(pre.rows[0].id, requester, true);
    } catch (error) { await client.query('ROLLBACK'); throw error; } finally { client.release(); }
  }

  async publishAssessment(id: string, requester: { id: string; role: UserRole }): Promise<unknown> { const ownership = requester.role === 'admin' ? '' : `AND created_by='${requester.id}'`; const result = await pool.query(`UPDATE assessments SET status='published',updated_at=NOW() WHERE (id=$1 OR paired_assessment_id=$1 OR id=(SELECT paired_assessment_id FROM assessments WHERE id=$1)) ${ownership} RETURNING id`, [id]); if (!result.rows[0]) throw new Error('Assessment not found or not owned by you.'); const assessment = await pool.query<{ subject: string }>(`SELECT subject FROM assessments WHERE id=$1`, [id]); const eligible = requester.role === 'admin' ? await pool.query<{ id: string }>(`SELECT id FROM users WHERE role='student' AND status='active'`) : await pool.query<{ id: string }>(`SELECT DISTINCT student_id AS id FROM enrollment_records WHERE teacher_id=$1 AND subject=$2 AND status='active'`, [requester.id, assessment.rows[0].subject]); if (eligible.rows.length) await this.assignAssessment(id, eligible.rows.map((row) => row.id), null, requester); return this.getAssessment(id, requester, true); }

  async assignAssessment(id: string, studentIds: string[], dueAt: string|null, requester: { id: string; role: UserRole }): Promise<unknown[]> {
    const assessment = await pool.query<{ subject:string; paired_assessment_id:string|null; assessment_type:'pre'|'post' }>(`SELECT subject,paired_assessment_id,assessment_type FROM assessments WHERE id=$1 AND status='published'`, [id]); if (!assessment.rows[0]) throw new Error('Only published assessments can be assigned.');
    const allowed = requester.role === 'admin' ? await pool.query(`SELECT id FROM users WHERE id=ANY($1::uuid[]) AND role='student' AND status='active'`, [studentIds]) : await pool.query(`SELECT DISTINCT e.student_id AS id FROM enrollment_records e WHERE e.teacher_id=$1 AND e.subject=$2 AND e.status='active' AND e.student_id=ANY($3::uuid[])`, [requester.id,assessment.rows[0].subject,studentIds]);
    const post = assessment.rows[0].assessment_type === 'pre' ? await pool.query<{ id: string }>(`SELECT id FROM assessments WHERE paired_assessment_id=$1`, [id]) : { rows: [] };
    const assessmentIds = assessment.rows[0].assessment_type === 'pre' && post.rows[0] ? [id, post.rows[0].id] : [id];
    for (const row of allowed.rows) { const enrollment = await pool.query(`SELECT id,teacher_id FROM enrollment_records WHERE student_id=$1 AND subject=$2 AND status='active' ORDER BY created_at DESC LIMIT 1`, [row.id,assessment.rows[0].subject]); for (const assessmentId of assessmentIds) await pool.query(`INSERT INTO assessment_assignments (assessment_id,student_id,teacher_id,enrollment_id,due_at) VALUES ($1,$2,$3,$4,$5) ON CONFLICT DO NOTHING`, [assessmentId,row.id,enrollment.rows[0]?.teacher_id || requester.id,enrollment.rows[0]?.id || null,dueAt]); }
    return this.listAssessmentAssignments(requester);
  }

  async listAssessmentAssignments(requester: { id: string; role: UserRole }): Promise<unknown[]> { const params: unknown[]=requester.role==='admin'?[]:[requester.id]; const where=requester.role==='admin'?'':requester.role==='teacher'?`WHERE x.teacher_id=$1`:`WHERE x.student_id=$1`; const result=await pool.query(`SELECT x.id,x.assessment_id AS "assessmentId",a.paired_assessment_id AS "pairedAssessmentId",x.student_id AS "studentId",su.full_name AS "studentName",x.teacher_id AS "teacherId",tu.full_name AS "teacherName",a.subject,a.grade_level AS "gradeLevel",a.assessment_type AS "assessmentType",a.title,x.due_at AS "dueAt",COALESCE(at.status,'not_started') AS "attemptStatus",at.score,paired.score AS "pairedScore",CASE WHEN at.score IS NOT NULL AND paired.score IS NOT NULL AND a.assessment_type='post' THEN at.score-paired.score END AS improvement FROM assessment_assignments x JOIN assessments a ON a.id=x.assessment_id JOIN users su ON su.id=x.student_id JOIN users tu ON tu.id=x.teacher_id LEFT JOIN assessment_attempts at ON at.assignment_id=x.id LEFT JOIN assessment_assignments px ON px.student_id=x.student_id AND px.assessment_id=a.paired_assessment_id LEFT JOIN assessment_attempts paired ON paired.assignment_id=px.id ${where} ORDER BY x.created_at DESC`, params); return result.rows; }

  async startAssessment(assignmentId: string, studentId: string): Promise<unknown> { const assignment=await pool.query(`SELECT x.assessment_id,a.assessment_type,a.paired_assessment_id FROM assessment_assignments x JOIN assessments a ON a.id=x.assessment_id WHERE x.id=$1 AND x.student_id=$2`,[assignmentId,studentId]); if(!assignment.rows[0]) throw new Error('Assessment assignment not found.'); if(assignment.rows[0].assessment_type==='post') { const pre=await pool.query(`SELECT at.status FROM assessment_assignments x JOIN assessment_attempts at ON at.assignment_id=x.id WHERE x.assessment_id=$1 AND x.student_id=$2`,[assignment.rows[0].paired_assessment_id,studentId]); if(pre.rows[0]?.status!=='submitted') throw new Error('Complete the pre-assessment before starting the post-assessment.'); } const existing=await pool.query(`SELECT id,status FROM assessment_attempts WHERE assignment_id=$1`,[assignmentId]); let attempt=existing.rows[0]; if(!attempt) { attempt=(await pool.query(`INSERT INTO assessment_attempts (assignment_id,student_id) VALUES ($1,$2) RETURNING id,status`,[assignmentId,studentId])).rows[0]; } if(attempt.status==='submitted') throw new Error('This assessment has already been submitted.'); const detail=(await this.getAssessment(assignment.rows[0].assessment_id,{id:studentId,role:'student'},false)) as Record<string, unknown>; const answers=await pool.query(`SELECT question_id AS "questionId",selected_choice_id AS "selectedChoiceId",answer_text AS "answerText" FROM assessment_answers WHERE attempt_id=$1`,[attempt.id]); return {...detail,assignmentId,attemptId:attempt.id,answers:answers.rows}; }

  async submitAssessment(assignmentId: string, studentId: string, answers: Array<{questionId:string;selectedChoiceId?:string|null;answerText?:string|null}>): Promise<unknown> { const check=await pool.query(`SELECT at.id,at.status,a.id AS assessment_id FROM assessment_attempts at JOIN assessment_assignments x ON x.id=at.assignment_id JOIN assessments a ON a.id=x.assessment_id WHERE at.assignment_id=$1 AND x.student_id=$2`,[assignmentId,studentId]); if(!check.rows[0]) { await this.startAssessment(assignmentId,studentId); return this.submitAssessment(assignmentId,studentId,answers); } if(check.rows[0].status==='submitted') throw new Error('This assessment has already been submitted.'); const attemptId=check.rows[0].id; const client=await pool.connect(); try { await client.query('BEGIN'); let earned=0,total=0; const questions=(await client.query(`SELECT q.id,q.points,q.question_type,q.accepted_answers,(SELECT c.id FROM assessment_choices c WHERE c.question_id=q.id AND c.is_correct=TRUE LIMIT 1) AS correct_choice FROM assessment_questions q WHERE q.assessment_id=$1`,[check.rows[0].assessment_id])).rows; for(const q of questions){const answer=answers.find((x)=>x.questionId===q.id); const text=String(answer?.answerText||'').trim().toLowerCase(); const correct=q.question_type==='identification'?q.accepted_answers.some((x:string)=>x.trim().toLowerCase()===text):answer?.selectedChoiceId===q.correct_choice; const points=correct?Number(q.points):0; total+=Number(q.points); earned+=points; await client.query(`INSERT INTO assessment_answers (attempt_id,question_id,selected_choice_id,answer_text,is_correct,points_earned) VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT (attempt_id,question_id) DO UPDATE SET selected_choice_id=EXCLUDED.selected_choice_id,answer_text=EXCLUDED.answer_text,is_correct=EXCLUDED.is_correct,points_earned=EXCLUDED.points_earned`,[attemptId,q.id,answer?.selectedChoiceId||null,answer?.answerText||null,correct,points]); } const updated=await client.query(`UPDATE assessment_attempts SET status='submitted',score=$1,total_points=$2,submitted_at=NOW(),updated_at=NOW() WHERE id=$3 RETURNING score,total_points,submitted_at`,[total?Math.round((earned/total)*100):0,total,attemptId]); const review=(await client.query(`SELECT aa.question_id AS "questionId",q.prompt AS question,aa.is_correct AS "isCorrect",COALESCE(selected.choice_text,aa.answer_text,'No answer') AS answer,COALESCE(correct.choice_text,q.accepted_answers[1],'No answer') AS "correctAnswer" FROM assessment_answers aa JOIN assessment_questions q ON q.id=aa.question_id LEFT JOIN assessment_choices selected ON selected.id=aa.selected_choice_id LEFT JOIN assessment_choices correct ON correct.question_id=q.id AND correct.is_correct=TRUE WHERE aa.attempt_id=$1 ORDER BY q.question_order`,[attemptId])).rows; await client.query('COMMIT'); return {attemptId,score:Number(updated.rows[0].score),totalPoints:Number(updated.rows[0].total_points),submittedAt:updated.rows[0].submitted_at,answerReviewEnabled:true,review}; } catch(error){await client.query('ROLLBACK');throw error;} finally{client.release();} }

  async getAssessmentAnalytics(requester: { id: string; role: UserRole }): Promise<unknown> { const rows=await this.listAssessmentAssignments(requester) as Array<Record<string,unknown>>; const submitted=rows.filter((r)=>r.attemptStatus==='submitted'&&r.score!==null); const scores=submitted.map((r)=>Number(r.score)); const improvements=rows.filter((r)=>r.improvement!==null).map((r)=>Number(r.improvement)); return {totalAssessments:new Set(rows.map((r)=>r.assessmentId)).size,assignedCount:rows.length,completedCount:submitted.length,averageScore:scores.length?Math.round(scores.reduce((a,b)=>a+b,0)/scores.length):null,averageImprovement:improvements.length?Math.round(improvements.reduce((a,b)=>a+b,0)/improvements.length):null,preCount:rows.filter((r)=>r.assessmentType==='pre').length,postCount:rows.filter((r)=>r.assessmentType==='post').length,results:rows}; }

  async saveAssessmentAnswers(assignmentId: string, studentId: string, answers: Array<{questionId:string;selectedChoiceId?:string|null;answerText?:string|null}>): Promise<void> { const started=await this.startAssessment(assignmentId,studentId) as {attemptId:string}; for(const answer of answers) await pool.query(`INSERT INTO assessment_answers (attempt_id,question_id,selected_choice_id,answer_text) VALUES ($1,$2,$3,$4) ON CONFLICT (attempt_id,question_id) DO UPDATE SET selected_choice_id=EXCLUDED.selected_choice_id,answer_text=EXCLUDED.answer_text`,[started.attemptId,answer.questionId,answer.selectedChoiceId||null,answer.answerText||null]); }
