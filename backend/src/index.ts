...
const updateProfileSchema = z
  .object({
    ...
    birthdate: z.preprocess((value) => value === '' ? null : value, z.string().date().nullable().optional()),
...
app.post('/api/cycling-quest/runs/complete', requireAuth, requireRole('student'), async (req: AuthenticatedRequest, res, next) => {
  try { const input = z.object({ score: z.coerce.number().int().min(0).max(10000), bossCorrectCount: z.coerce.number().int().min(0).max(7) }).parse(req.body); res.status(201).json(await service.completeCyclingQuestRun(req.auth?.sub || '', input)); } catch (error) { next(error); }
});
...
app.get('/api/cycling-quest/stats', requireAuth, requireRole('student'), async (req: AuthenticatedRequest, res, next) => {
  try { res.json(await service.getCyclingQuestStats(req.auth?.sub || '')); } catch (error) { next(error); }
});
